<template>
  <transition-group name="fade" tag="div" @beforeEnter="beforeEnter" @enter="enter" @leave="leave">
    <div
      class="row d-none"
      v-for="(item, index) in products"
      v-if="item.price<=Number(maximum)"
      :key="item.id"
      :data-index="index"
    >
      <div class="col-sm-1 m-auto">
        <div class="btn btn-info" @click="$emit('add', item)">+</div>
      </div>

      <div class="col-sm-4">
        <img class="img-fluid d-block" :src="item.image" :alt="item.name" />
      </div>
      <div class="col">
        <h3 class="text-info">{{ item.name }}</h3>
        <p class="mb-0">{{ item.description }}</p>

        <!--Price Template-->
        <div class="h5 float-right"></div>
        <!--4. We have one problem, the values that we are passing in are not of the correct type-->
        <!--5. Typecast the Numbers using Number()-->
        <price
          :value="Number(item.price)"
          :prefix="item.prefix"
          :precision="Number(2)"
          :conversion="Number(.87)"
        ></price>
      </div>
    </div>
    <!-- </div> -->
  </transition-group>
</template>

<script>
import Price from "./Price.vue";

export default {
  name: "product-list",
  components: { Price },
  props: ["products", "maximum"],
  methods: {
    beforeEnter: function (el) {
      el.className = "d-none";
    },
    enter: function (el) {
      var delay = el.dataset.index * 100;
      setTimeout(function () {
        el.className =
          "row d-flex mb-3 align-items-center animated fadeInRight";
      }, delay);
    },
    leave: function (el) {
      var delay = el.dataset.index * 100;
      setTimeout(function () {
        el.className =
          "row d-flex mb-3 align-items-center animated fadeOutRight";
      }, delay);
    },
  },
};
</script>

<style>
</style>